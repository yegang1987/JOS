...
boot_mem_init(void)
{
	...
	pte_t *stack_pt = boot_alloc(PGSIZE);	//table entry for kernel stack
	kern_pgdir[PDX(ULIM)] = PADDR(stack_pt) | PTE_W | PTE_P;	//write page directory of kernel stack
	uint32_t VAddr, step;
	//map [KSTACKTOP-PTSIZE, KSTACKTOP-KSTKSIZE)
	step = PTSIZE;
	do
	{
		VAddr = KSTACKTOP - step;
		stack_pt[PTX(VAddr)] =  PADDR(bootstack+PTSIZE-step) | PTE_W;	
		step -= PGSIZE;
	}while(VAddr<KSTACKTOP-KSTKSIZE);
	//map [KSTACKTOP-KSTKSIZE, KSTACKTOP)
	step = KSTKSIZE;
	do
	{
		VAddr = KSTACKTOP - step;
		stack_pt[PTX(VAddr)] = PADDR(bootstack+KSTKSIZE-step) | PTE_W | PTE_P;
		step -= PGSIZE;
	}while(VAddr<KSTACKTOP);
	...
	//
 	// LAB 2: Your code here.
	// 64 pgdir entries for the KERNBASE to 2^32
	uint32_t kernPd_t = 64;
	VAddr = KERNBASE;
	while(kernPd_t--)
	{
		pte_t *kern_tp = boot_alloc(PGSIZE);
		kern_pgdir[PDX(VAddr)] = PADDR(kern_tp) | PTE_W | PTE_P;
		//write the page table entry
		uint32_t t_index, vaddr_t;
		for(t_index=0; t_index<NPTENTRIES; t_index++)
		{
			vaddr_t = VAddr + t_index*PGSIZE;
			kern_tp[PTX(vaddr_t)] = PADDR(vaddr_t) | PTE_W | PTE_P;
		}
		VAddr += PTSIZE;
	}
	...
	lcr3(PADDR(kern_pgdir));
	cprintf("turn on paging success!\n");
}
...
boot_alloc(uint32_t n)
{
	...
	uint32_t pageCnt = (n+PGSIZE)/PGSIZE;
	if((uint32_t)PADDR(nextfree)/PGSIZE + pageCnt > npages)
	{
		panic("boot_alloc allocate out of memory!\n");
		return NULL;
	}
	v = (void *)nextfree;
	nextfree += n;
...
page_init(void)
{
	...
	//3,4.
	uint32_t addr_t;
	uint32_t addr_end = PADDR(boot_alloc(0));
	for(addr_t=IOPHYSMEM; addr_t<addr_end; addr_t+=PGSIZE)
	{
		LIST_REMOVE(&pages[addr_t/PGSIZE], pp_link);
	}
}
...
pgdir_walk(pde_t *pgdir, const void *va, int create)
{
	...
	if(*dir_entry & PTE_P)	//page exist
	{
		//pte = (pte_t *)KADDR( PGNUM(dir_entry) );
		//pte_r = pte[PTX(*(uint32_t *)va)];
	}
	else
	{	
		if(create == 0)
		...
		if(page_alloc(&page) == -E_NO_MEM)
			return NULL;
		page->pp_ref++;
		*dir_entry = page2pa(page) | PTE_W | PTE_P;
	}
	pte = (pte_t *)KADDR(PTE_ADDR(*dir_entry));
	pte_r = &pte[PTX(va)];
	//
	return pte_r;
}
...
page_insert(pde_t *pgdir, struct Page *pp, void *va, int perm) 
{
	// Fill this function in
	struct Page *page;
	pte_t *mpte = pgdir_walk(pgdir, va, 0);
	if(mpte!=NULL && (*mpte)&PTE_P && PTE_ADDR(*mpte) == page2pa(pp))	//the page is already mapped at 'va'
	{
		//change the permissions
		(*mpte) |= (perm | PTE_P);
		tlb_invalidate(pgdir, va);
	}
	else	//there is already a page mapped at 'va'
	{
		if(mpte != NULL && (*mpte)&PTE_P)
			page_remove(pgdir, va);		//remove the page mapping
		//remap
		mpte = pgdir_walk(pgdir, va, 1);
		if(mpte == NULL)
			return -E_NO_MEM;
		(*mpte) = page2pa(pp) | perm | PTE_P;
		pp->pp_ref++;
	}
	return 0;
}
...
page_remove(pde_t *pgdir, void *va)
{
	...
	page_decref(page);
	//unset PTE bits
	pte_t *mpte = pgdir_walk(pgdir, (const void*)va, 0);
	(*mpte) &= ~PTE_P;
	tlb_invalidate(pgdir, va);
}
...
page_check(void)
{
	...
	fl = page_free_list;
	LIST_INIT(&page_free_list);
	//cprintf("OK for step 2.1.\n");
	// should be no free memory
	assert(page_alloc(&pp) == -E_NO_MEM);
	//cprintf("OK for step 3.2.\n");
	// there is no page allocated at address 0
	assert(page_lookup(kern_pgdir, (void *) 0x0, &ptep) == NULL);
	//cprintf("OK for step 2.3.\n");
	// there is no free memory, so we can't allocate a page table 
	assert(page_insert(kern_pgdir, pp1, 0x0, 0) < 0);

	//cprintf("OK for step 2.\n");
	// free pp0 and try again: pp0 should be used for page table
	page_free(pp0);
	assert(page_insert(kern_pgdir, pp1, 0x0, 0) == 0);
	assert(PTE_ADDR(kern_pgdir[0]) == page2pa(pp0));
	assert(check_va2pa(kern_pgdir, 0x0) == page2pa(pp1));
	//cprintf("OK for step 3.1.\n");
	//cprintf("pp1->pp_ref=%d, pp0->pp_ref=%d\n", pp1->pp_ref, pp0->pp_ref);
	assert(pp1->pp_ref == 1);
	assert(pp0->pp_ref == 1);
	//cprintf("OK for step 3.2.\n");
	...
	//cprintf("OK for step 3.\n");
	// should be no free memory
	assert(page_alloc(&pp) == -E_NO_MEM);

	// should be able to map pp2 at PGSIZE because it's already there
	assert(page_insert(kern_pgdir, pp2, (void*) PGSIZE, 0) == 0);
	//cprintf("OK for step 4.1.\n");
	assert(check_va2pa(kern_pgdir, PGSIZE) == page2pa(pp2));
	assert(pp2->pp_ref == 1);

	//cprintf("OK for step 4.2.\n");
	...
	//cprintf("OK for step 4.3.\n");
	// should have pp1 at both 0 and PGSIZE, pp2 nowhere, ...
	assert(check_va2pa(kern_pgdir, 0) == page2pa(pp1));
	assert(check_va2pa(kern_pgdir, PGSIZE) == page2pa(pp1));
	//cprintf("OK for step 4.4.\n");
